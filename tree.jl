...
student_name = "Henry" # write your name here and run the cell
...
begin
	function too_close(tree_i::Tree, tree_j::Tree, R::Float64)
		if ((tree_i.x - tree_j.x)^2 + (tree_i.y - tree_j.y)^2)^0.5 < 2*R
			return true
		else
			return false
		end
	end
	function too_close(proposed_tree::Tree, planted_trees::Array{Tree, 1}, R::Float64)
		for planted_tree in planted_trees
			if too_close(proposed_tree, planted_tree, R)
				return true
			end
		end
		return false
...
# ╔═╡ d4a32f60-04fc-11eb-3475-1bf016138ef4
begin
	test_tree_1 = Tree(1.1,1.1)
	test_tree_2 = Tree(6.0,1.1)
	too_close(test_tree_1, test_tree_2, R)
end
...
function plant_trees(R::Float64, nb_trees::Int, land::Land)
	trees = Array{Tree, 1}(undef, nb_trees)
	trees[1] = Tree(rand() * land.L, rand() * land.W)
	for n in 1:(nb_trees-1)
		potential_tree = Tree(rand() * land.L, rand() * land.W)
		while too_close(potential_tree, trees[1:n], R)
			potential_tree = Tree(rand() * land.L, rand() * land.W)
		end
		trees[n+1] = potential_tree
	end
	return trees::Array{Tree, 1}
end
...
begin
	function draw_circle(x::Float64, y::Float64, R::Float64)
		plt.gcf().gca().add_artist(plt.Circle((x,y), R, fill=false, ls="--",lw=0.5,
				color="grey"))
	end
	function viz_tree(trees::Array{Tree, 1}, land::Land)
		clf()
		scatter([tree.x for tree in trees], [tree.y for tree in trees],
			color="green", marker="x")
		[draw_circle(tree.x, tree.y, R) for tree in trees]
		plt.gcf().gca().add_artist(plt.Rectangle((0,0), 100, 50, fill=false, lw=0.5, zorder=0, alpha=0.5))
