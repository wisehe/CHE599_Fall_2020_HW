...
estimate_prob_needle_crosses_line(10000, floor, L)

# ╔═╡ 9aeee1a8-04d2-11eb-1bee-092c03afd72d
isapprox(exact_p(floor, L), estimate_prob_needle_crosses_line(1000000, floor, L), atol=1e-3)

# ╔═╡ 5ca13928-045f-11eb-2af6-914d599250e6
percentile(rand(1000), 95.0)

# ╔═╡ 3b01794e-045b-11eb-3ee5-cdebb0f4d285
begin
	nb_tosses_range = [10, 50, 100, 500, 1000, 5000, 10000, 50000, 100000]
	nb_simulations = 100
	errors = zeros(2, length(nb_tosses_range))
	averages = zeros(length(nb_tosses_range))
	
	for (n, nb_tosses) in enumerate(nb_tosses_range)
		estimates = [estimate_prob_needle_crosses_line(nb_tosses, floor, L) for _ in 1:nb_simulations]
		averages[n] = mean(estimates)
		errors[:,n] = abs.(percentile(estimates,[5,95]) - repeat([averages[n]], 2))
	end
end
...
begin
	figure()
	errorbar(nb_tosses_range, averages, fmt="o", yerr=errors, capsize=5)
	xlabel("# tosses")
	ylabel("probability of cross")
	axhline(y=1/π, color="gray", linestyle="--")
	xscale("log")
	gcf()
end
